...
pdfjsLib.GlobalWorkerOptions.workerSrc = pdfWorker;
        ...
        function App() {
                ...
                const resetSaleForm = () => {
                    setCurrentItems([]); setIsInvoice(false); setClientName(''); setRfc(''); setCp('');
                    ...
                    <div className="max-w-5xl mx-auto px-4 sm:px-6">
                            ...
                            <div className="animate-fade-in space-y-6">
                                ...
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        <input type="text" placeholder="Público en General (Nombre / Razón Social)" value={clientName} onChange={e => setClientName(e.target.value)} className="neumorphic-input w-full p-3 rounded-xl font-medium" />
                                        {isInvoice && (<><input type="text" placeholder="RFC" value={rfc} onChange={e => setRfc(e.target.value.toUpperCase())} className={`neumorphic-input w-full p-3 rounded-xl font-medium ${!isValidRFC(rfc) && rfc.length > 0 ? 'border-red-300 text-red-600' : ''}`} maxLength={13} /><input type="text" placeholder="Código Postal" value={cp} onChange={e => setCp(e.target.value)} className="neumorphic-input w-full p-3 rounded-xl font-medium" maxLength={5} /><select value={clientRegime} onChange={e => setClientRegime(e.target.value)} className="neumorphic-input w-full p-3 rounded-xl font-bold text-slate-900">{FISCAL_REGIMES.map(r => <option key={r.code} value={r.code} className="text-slate-900 font-bold">{r.label}</option>)}</select><select value={usoCfdi} onChange={e => setUsoCfdi(e.target.value)} className="neumorphic-input w-full p-3 rounded-xl font-bold text-slate-900">{USOS_CFDI.map(u => <option key={u.code} value={u.code} className="text-slate-900 font-bold">{u.label}</option>)}</select></>)}
                                        <select value={salesChannel} onChange={e => setSalesChannel(e.target.value)} className="neumorphic-input w-full p-3 rounded-xl font-bold text-slate-900">{SALES_CHANNELS.map(c => <option key={c} value={c} className="text-slate-900 font-bold">{c}</option>)}</select>
                                        <select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} className="neumorphic-input w-full p-3 rounded-xl font-bold text-slate-900">{PAYMENT_METHODS.map(m => <option key={m} value={m} className="text-slate-900 font-bold">{m}</option>)}</select>
                        ...
                        {view === 'config' && (
                            <div className="animate-fade-in space-y-6 max-w-2xl mx-auto">
                                ...
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                                    <h3 className="font-bold text-lg text-slate-900 mb-4 flex items-center gap-2"><ShieldCheck size={20} className="text-indigo-500"/> Legal y Privacidad</h3>
